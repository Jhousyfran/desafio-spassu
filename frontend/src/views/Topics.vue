...
const fetchItems = async (p = 1) => {
  const { data } = await api.get('/topics', { params: { page: p } });
  items.value = data;
};
...
<template>
  <h1 class="h3 mb-4 text-capitalize">Assunto</h1>
  ...
  <TopicForm v-if="editing" :model-value="editing" @saved="fetchItems" @close="editing = null" />
</template>
